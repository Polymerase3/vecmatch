...
#' @srrstats {G2.0a} Expectations on input lengths are documented in the
#'   help pages for specific functions, where arguments
#'   are explicitly described as accepting either single values (recycled
#'   internally) or vectors of length equal to the number of treatment
#'   combinations / rows in the optimization grid.
#' @srrstats {G2.1a} Expected data types of vector inputs are documented in the
#'   `@param` and `@details` sections of user-facing functions. For example,
#'   `estimate_gps()` documents that the treatment must be a factor-like
#'   variable and covariates numeric or factor, `csregion()` requires a `gps`
#'   data frame, `match_gps()` expects numeric GPS columns and character
#'   scalars for tuning arguments, and plotting functions such as `raincloud()`
#'   and `mosaic()` describe the types of `data`, `y`, `group`, and `facet`.
#' @srrstats {G2.3} vecmatch validates univariate character options against a
#'   fixed set of allowed values. Arguments such as `method`, `link`,
#'   `density_scale`, `smd_type`, `significance`, `order`, and `type`
#'   are restricted using `match.arg()`-equivalent checks based on explicit
#'   membership tests and informative error messages.
#' @srrstats {G2.3a} For univariate character inputs, vecmatch uses explicit
#'   membership checks (e.g. `arg %nin% allowed_values` combined with
#'   `.chk_cond()`) to only permit documented choices, functionally equivalent
#'   to `match.arg()`.
#' @srrstats {G2.3b} Where appropriate (e.g. for `type`, `statistic`,
#'   `smd_type`, `significance`), inputs are normalised with `tolower()`
#'   so that matching is case-insensitive. In other cases, parameters are
#'   treated as strictly case-sensitive and this is documented in the
#'   corresponding `@param` sections.
...
#' @noRd
#'
#' @srrstatsNA {G1.5} Currently no associated publications or external
#'   performance claims exist for `vecmatch`
#' @srrstatsNA {G1.6} `vecmatch` currently makes no formal performance
#'   comparisons with alternative R implementations, because for settings
#'   involving matching of multiple treatment groups, there is no alternative R
#'   implementations
NULL
