...
names(vec_col_list) <- c("one", "two", "three", "four", "five",
...
scale_fill_vecmatch <- function(n, type = c('discrete', 'continuous')) {
  if (type == 'discrete') {
    ggplot2::scale_fill_manual(values = .generate_palettes(n = n,
                                                           type = 'discrete'))
  } else {
   ggplot2::scale_fill_gradientn(colors = .generate_palettes(n = 3,
                                                             type = 'continuous'))
  }
}
...
scale_color_vecmatch <- function(n, type = c('discrete', 'continuous')) {
  if (type == 'discrete') {
    ggplot2::scale_colour_manual(values = .generate_palettes(n = n,
                                                           type = 'discrete'))
  } else {
    ggplot2::scale_colour_gradientn(colors = .generate_palettes(n = 3,
                                                              type = 'continuous'))
  }
}
