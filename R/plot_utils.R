...
scale_y_product <- function(coords) {
  ...
  for (i in 1:nrow(coords_sub)) {
    ...
    label_position$name[i] <- if (single_aes) {
        ...
        as.character(sub_data[, which(names(sub_data) == 'level') + 1])  # Last column for multiple aesthetics
      }
  }
...
#--solution by https://dewey.dunnington.ca/post/2018/modifying-facet-scales-in-ggplot2/
#--simplified and modified
scale_facet <- function(which_facet, scale_arg) {
  structure(list(which_facet = which_facet, scale_arg = scale_arg), class = 'scale_facet')
}
...
FacetWrapScales <- ggplot2::ggproto('FacetWrapScales',
    ...
    init_scales = function(self, layout, x_scale = NULL, y_scale = NULL, params) {
      ...
      if(is.null(params$scales_custom)) return(scales)
...
facet_wrap_scales <- function(..., scales_custom = NULL) {
  ...
  if(inherits(scales_custom, 'scale_facet')) scales_custom = list(scales_custom)
